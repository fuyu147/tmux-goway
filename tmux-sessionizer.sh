#!/usr/bin/env bash
#
# script taken from : https://github.com/ThePrimeagen/tmux-sessionizer/
# this version is modified by fuyu147.
#
#
...
while [[ "$#" -gt 0 ]]; do
    ...
    -h | --help)
        ...
        echo "  --previous             Switch to the last used session."
        exit 0
        ;;
    -s | --session)
        session_idx="$2"
...
handle_previous_session() {
    ...
    if [[ -n "$TMUX" ]]; then
        # We are inside tmux, switch to the last session for this client
        tmux switch-client -l
    else
        # We are outside tmux, attach to the most recently active session
        last_session_name=$(tmux list-sessions -F '#{session_activity} #{session_name}' | sort -rn | head -n 1 | cut -d' ' -f2-)
        if [[ -n "$last_session_name" ]]; then
            tmux attach-session -t "$last_session_name"
        else
            echo "No previous session to attach to."
            exit 1
        fi
    fi
    exit 0
...
if [[ ! -z $session_cmd ]]; then
    ...
    selected=$(find_dirs | fzf --tmux=top,100%,40%)
fi
